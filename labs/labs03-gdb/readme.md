...
* เช็คเวอร์ชันของ GDB เพื่อตรวจสอบว่า GDB ถูกติดตั้งในเครื่องแล้ว
   ```bash
   gdb --version
   ```
...
เนื่องจาก Apple ได้เปลี่ยนสถาปัตยกรรมของ CPU ใน Mac รุ่นใหม่จาก X86_64 ไปเป็นแบบ ARM ทำให้การใช้งาน GDB บน Mac ARM
อาจมีความซับซ้อนกว่ามากๆ ผู้ใช้ Mac ARM นั้นสามารถใช้โปรแกรม `lldb` ในการ debugging
...
ถ้าหากว่าในเครื่องได้มีโปรแกรมอยู่แล้ว จะขึ้นข้อความดังนี้
   ...
   ```
   lldb-XXXX.X.XXX.X
   Apple Swift version X.X (swiftlang-X.X.X.X clang-XXXX.X.X.X)
   ```

หากไม่ขึ้นสามารถติดตั้ง lldb ได้ผ่าน Command Line Tools โดยใช้คำสั่ง `xcode-select --install`
...
2. **Start the Debugger:**
   ...
   ในโค้ดตัวอย่าง main.c มีบรรทัดทั้งหมด 15 บรรทัด เราจะเห็นว่าในการรัน `list` ครั้งแรกจะแสดงโค้ดมาแค่ 10
...
3. **Set Breakpoints:**
   ...
   ให้น้องลองพิมพ์คำสั่งที่ใช้สำหรับสร้าง breakpoint ที่บรรทัดที่ 6 และ 11
...
5. **Check Variable Values:**
   ...
   นอกเหนือจากการปริ้นตัวแปรทีละตัว น้องๆ สามารถใข้คำสั่ง `info locals` ช่วยให้เรามองเห็นภาพรวมของตัวแปร local
   ทั้งหมดภายในขอบเขตของฟังก์ชันที่กำลังทำงานอยู่ได้อย่างรวดเร็ว
...
6. **Step Through the Code:**
   ...
   | คำสั่ง     | การทำงาน                                        | เหมาะสำหรับ                                                          |
      |------------|-------------------------------------------------|----------------------------------------------------------------------|
   | `next`     | ทำงานบรรทัดถัดไป ไม่เข้าไปในฟังก์ชัน            | ข้ามฟังก์ชันที่มั่นใจว่าถูกต้อง                                      |
   | `step`     | ทำงานบรรทัดถัดไป เข้าไปในฟังก์ชันที่ถูกเรียกใช้ | ตรวจสอบการทำงานของฟังก์ชันอย่างละเอียด                               |
   | `continue` | ทำงานต่อเนื่องจนกว่าจะพบ breakpoint             | ผ่านโค้ดที่ไม่สนใจ, ไปยัง breakpoint ถัดไป                           |
   | `finish`   | ทำงานต่อจนจบฟังก์ชันปัจจุบัน                    | ออกจากฟังก์ชันปัจจุบันอย่างรวดเร็ว เมื่อตรวจสอบส่วนที่เหลือเสร็จแล้ว |
...
7. **Watch the variable:**
   ...
   คำสั่ง `watch` ใน GDB เป็นเครื่องมือที่มีประโยชน์อย่างมากในการตรวจสอบการเปลี่ยนแปลงของค่าในตัวแปรระหว่างการ Debugging
   ต่างจากคำสั่ง print ที่แสดงค่าของตัวแปร ณ จุดที่รันคำสั่งเท่านั้น

   `watch` จะคอยตรวจสอบนิพจน์ที่เรากำหนดไว้ตลอดเวลา และจะ**หยุดการทำงานของโปรแกรมทันทีที่ค่าของนิพจน์นั้นเปลี่ยนแปลง**
   มี Syntax ดังนี้
   ...
   จากตัวอย่างข้างบนที่น้องได้ทดลอง debugging มาและหยุดที่บรรทัดที่ 11 ให้น้องลอง watch ตัวแปร `a` ดูโดยใช้คำสั่ง
   ...
   เมื่อน้องรันคำสั่งนี้แล้ว มันจะสร้าง Watchpoint ขึ้นมา โดยสามารถเช็ค Watchpoint ทั้งหมดได้จากคำสั่ง
   ```gdb
   ...
   อีกความเทพเลยก็คือ ปกติถ้าน้องรันคำสั่ง `continue` ไปแล้ว โปรแกรมก็จะทำงานไปเรื่อยๆ จนถึงจุด Breakpoint
   อันต่อไปหรือถ้าไม่มีก็จะทำงานจนสิ้นสุดโปรแกรม แต่ถ้าน้องใส่ `watch` เข้าไปแล้ว คำสั่ง `continue` จะทำงานไปเรื่อยๆ
   จนกว่าค่าของตัวแปรที่น้องใส่จะถูกเปลี่ยนแปลง
   ...
   จากการทำงานของคำสั่ง `continue` ด้านบนคือเกิดการเปลี่ยนแแปลงของตัวแปร a ที่โค้ด `a++` (เรียกใช้ a
   ก่อนแล้วค่อยเพิ่มค่า +1)
   ทำให้โปรแกรมหยุดทำงานทันทีเมื่อค่าตัว a ได้ถูกเปลี่ยนแปลงซึ่งถ้าน้องรันคำสั่ง `continue` อีกรอบนึง
   ...
   ค่าของ a จากเดิมที่เป็น 13 ได้กลายไปเป็น 79 เพราะว่าค่า a ได้ถูกเปลี่ยนแปลงอีกรอบที่โค้ดส่วน `a = a++ + ++d;` (a =
   13 + 66) จึงทำให้โปรแกรมหยุดทำงาน

   ![gdb-command-tutorial.jpg](files/run-06-watch04.png)

   ให้น้องลองดูการเปลี่ยนแปลงของค่า a ไปเรื่อยๆ จนจบการทำงานของโปรแกรม
...
`gdb --tui` เข้ามาช่วยให้การใช้งาน GDB ง่ายขึ้น โดยการแสดงผลแบบกราฟิก แบ่งหน้าจอออกเป็นส่วนๆ ให้ดูง่าย คล้ายๆกับมี IDE
มาช่วย
...
น้องๆ สามารถใช้ `--tui` ในการลอง debug โค้ดเล่นๆ หรือใช้ทำ Quiz ก็ได้

![img.png](files/run-07-tui01.png)
...
- **Tutorialspoint GDB tutorial:** https://www.tutorialspoint.com/gnu_debugger/index.htm